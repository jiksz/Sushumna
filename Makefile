...
docker:
	mkdir -p /tmp/sushumna/build
	cp Dockerfile-build /tmp/sushumna/build/Dockerfile
	docker build -t sushumna/build /tmp/sushumna/build
	docker run -ti -v ${PWD}:/sources -u sushumna sushumna/build bash -c  "make setup; make release"
	...
	cp Dockerfile-run admintool/release/Dockerfile; docker build -t sushumna/run admintool/release
