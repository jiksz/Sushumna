...
//! A multiplexed sushumna protocol
...
pub type sushumnaRequest = Vec<u8>;
pub type sushumnaResponse = Vec<u8>;

/// Our multiplexed line-based codec
pub struct sushumnaCodec;

/// Protocol definition
pub struct sushumnaProto;
...
/// +----------------+------------------------------+
///
impl Decoder for sushumnaCodec {
    type Item = sushumnaRequest;
    type Error = io::Error;
...
impl Encoder for sushumnaCodec {
    type Item = sushumnaResponse;
    type Error = io::Error;
...
impl<T: AsyncRead + AsyncWrite + 'static> ServerProto<T> for sushumnaProto {
    type Request = sushumnaRequest;
    type Response = sushumnaResponse;

    /// `Framed<T, sushumnaCodec>` is the return value of `io.framed(sushumnaCodec)`
    type Transport = Framed<T, sushumnaCodec>;
    type BindTransport = Result<Self::Transport, io::Error>;

    fn bind_transport(&self, io: T) -> Self::BindTransport {
        Ok(io.framed(sushumnaCodec))
    }
}
