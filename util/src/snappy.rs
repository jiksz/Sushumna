...
const sushumna_COMPRESS_SIZE: usize = 4 * 1024;
pub fn sushumna_compresse(input: Vec<u8>) -> Vec<u8> {
    if input.len() > sushumna_COMPRESS_SIZE { compress(&input) } else { input }
}

pub fn sushumna_decompress(input: Vec<u8>) -> Vec<u8> {
