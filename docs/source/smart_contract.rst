...
在sushumna中是使用 ``JSON-PRC`` 来发送交易的，发送创建合约交易之前，首先需要构造创建合约的Json配置文件 ``config_create.json`` ，其内容如下：
...
执行成功后，便会经过sushumna系统的处理，然后返回创建合约交易的执行结果。
...
执行成功后，便会经过sushumna系统的处理，然后返回调用合约交易的执行结果。
