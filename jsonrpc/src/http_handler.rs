...
use jsonrpc_types::response::{self as sushumna_response, RpcSuccess, RpcFailure};
                        ...
                        self.send_mq(topic, tx.into(), self.tx_responses.clone(), hash)
                            ...
                            .map(|data| {
                                     RpcSuccess {
                                         ...
                                         id: req_id,
                                         result: sushumna_response::ResponseBody::from(data), //TODO
                                     }
                                 })
                    }
                        ...
                        self.send_mq(topic, req.into(), self.responses.clone(), key)
                            ...
                            .map(|data| {
                                     RpcSuccess {
                                         jsonrpc: jsonrpc_version,
                                         id: req_id,
                                         result: sushumna_response::ResponseBody::from(data.result.expect("chain response error")), //TODO
                                     }
                                 })
                    }
                }
