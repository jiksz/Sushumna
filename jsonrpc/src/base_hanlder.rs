...
pub trait BaseHandler {
    fn select_topic(method: &String) -> String {
        let topic = if method.starts_with("sushumna_send") {
                        "jsonrpc.new_tx"
                    } else if method.starts_with("sushumna") || method.starts_with("eth") {
                        "jsonrpc.request"
...
mod test {
    ...
    fn test_get_topic() {
        ...
        assert_eq!(Handler::select_topic(&"sushumna_send".to_string()), "jsonrpc.new_tx".to_string());
        assert_eq!(Handler::select_topic(&"sushumna".to_string()), "jsonrpc.request".to_string());
