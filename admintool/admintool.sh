...
do
    ...
    cp sushumna $DATA_PATH/node$ID/
    cp $DATA_PATH/.env $DATA_PATH/node$ID/
    case "$OSTYPE" in
        darwin*)  
            sed -ig "s/dev/node$ID/g" $DATA_PATH/node$ID/.env
            sed -ig "s/tendermint/$CONSENSUS_NAME/g" $DATA_PATH/node$ID/sushumna
            ;; 
        *)       
            sed -i "s/dev/node$ID/g" $DATA_PATH/node$ID/.env
            sed -i "s/tendermint/$CONSENSUS_NAME/g" $DATA_PATH/node$ID/sushumna
            ;;
    esac
